...
/**
 *On double click make string editable
 */

const handleDblClickItemName = function () {
  $('.js-shopping-list').on('dblclick', '.shopping-item', (evt) => {
    console.log();
    let x = 1;
    $(evt.currentTarget).html(
      `<form class='js-new-name-form'><input class='js-edited-name' type = 'text' value='${$(
        evt.currentTarget
      ).text()}' > </form>`
    );
  });
...
const updateItemName = function () {
  $('.js-shopping-list').on('submit', '.js-new-name-form', function (evt) {
    evt.preventDefault();
    let id = getItemIdFromElement(this);
    let item = store.items.find((item) => item.id === id);
    item.name = $('.js-edited-name').val();
    render();
  });
};
